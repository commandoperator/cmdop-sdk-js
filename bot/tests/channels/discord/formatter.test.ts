...
import { DiscordFormatter } from '../../../src/channels/discord/formatter.js';
import {
  BotError,
...
describe('DiscordFormatter', () => {
  ...
  describe('formatText', () => {
    ...
    it('escapes tilde', () => {
      expect(fmt.formatText('~~strikethrough~~')).toBe('\\~\\~strikethrough\\~\\~');
    });

    it('escapes backtick', () => {
      expect(fmt.formatText('`code`')).toBe('\\`code\\`');
    });
    ...
    it('passes through dot and hyphen unchanged', () => {
      // Discord does not require escaping . or -
