/**
 * Zod schema for PatchedMachineRequest
 *
...
export const PatchedMachineRequestSchema = z.object({
  ...
  uid: z.number().int().min(0.0).max(2147483647.0).nullable().optional(),
  is_root: z.boolean().optional(),
  home_dir: z.string().max(500).optional(),
  cpu_model: z.string().max(255).optional(),
  cpu_count: z.number().int().min(0.0).max(32767.0).optional(),
  total_ram_bytes: z.number().int().min(-9.223372036854776e+18).max(9.223372036854776e+18).optional(),
  ...
  uptime_seconds: z.number().int().min(-9.223372036854776e+18).max(9.223372036854776e+18).optional(),
  process_count: z.number().int().min(-2147483648.0).max(2147483647.0).optional(),
  agent_version: z.string().max(20).optional(),
  agent_token: z.string().min(1).max(255).optional(),
})

/**
 * Infer TypeScript type from Zod schema
 */
export type PatchedMachineRequest = z.infer<typeof PatchedMachineRequestSchema>